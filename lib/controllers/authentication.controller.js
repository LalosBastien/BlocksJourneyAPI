...
export default class AuthenticationController {
    ...
    static async register(req, res) {
        ...
        try {
            ...
            if (profId != '') {
                const professor = _getProfessorFromId(profId);
                if (professor != null) {
    ...
    static async renewStudent(req, res) {
        try {
            ...
            const student = userService.findById(req.params.id);
            if (student.profId != req.user.id) {
                return res.status(status.UNAUTHORIZED).json({
                    message: 'Action not authorized'
                })
            }
            const newPassword = generatePasswordToken();
            userService.updatePassword(student.id, hashPassword(newPassword));
            console.log("renew end; pwd=" + newPassword);
            return res.status(status.OK).json({ password: newPassword});
        } catch (error) {
