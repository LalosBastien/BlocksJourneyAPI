...
export default class ChapterController {
    ...
    static async getAll(req, res, next) {
        try {
            const list = await chapterService.getAll();
    ...
    static async findById(req, res, next) {
        try {
            ...
            if (stats.length === 0) {
                chapterService.createStats(chapter, req.user);
            }
            const chapterInfo = chapters[`chapter${req.params.id}`];
            if (!chapterInfo) {
    ...
    static async levels(req, res, next) {
        try {
            const chapter = req.chapter;
            res.json(await chapterService.findLevelsOf(chapter));
        } catch (err) {
            next(err);
        }
    }
