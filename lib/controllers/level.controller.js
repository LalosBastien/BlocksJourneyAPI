...
import { levelService, userService, objectifService } from '../services';
...
export default class LevelController {
    ...
    static async findById(req, res, next) {
        try {
            ...
            if (!levelInfo) {
                return res.status(status.NOT_FOUND).json({ message: 'Level not found' });
            }
            res.json({ message: { level, levelInfo } });
        } catch (err) {
            next(err);
        }
    }
    ...
    static async update(req, res, next) {
        try {
            if ((await levelService.update(req.params.id, req.body))[0] > 0) {
                return res.status(status.OK).json({ message: 'Level updated successfully' });
            }
            return res.status(status.NOT_FOUND).json({ message: 'Level did not update' });
        } catch (err) {
            next(err);
        }
    ...
    static async validate(req, res, next) {
        try {
            ...
            res.json({ message: 'Level validated successfully' });
        } catch (err) {
            next(err);
        }
    }
