/* eslint-disable no-await-in-loop, no-mixed-operators */
...
export default class StatsController {
    ...
    static async ladder(req, res, next) {
        try {
            const students = await profService.students(req.user);
            if (!students) throw new Error('Data Unavailable');
            let ladderWithProgress = await Promise.all(Object.values(students).map(async (s) => {
    ...
    static async statsGlobal(req, res) {
        try {
            const studentList = await profService.students(req.user).map(s => s.dataValues);
            let studentsStats = await Promise.all(studentList
                .map(s => statService.historyByUser(s.id)));
            studentsStats = [].concat(...studentsStats);
            ...
            const progressionByStudents = studentList.map((student) => {
                ...
                const data = studentStats.reduce((acc, cur) => {
                    const { doneIds, inProgressIds } = acc;
                    if (cur.status === 'success' && !doneIds.includes(cur.levelId))
                    { doneIds.push(cur.levelId); }
                    else if ((cur.status === 'in_progress' || cur.status === 'failed') &&
                        !inProgressIds.includes(cur.levelId) && !doneIds.includes(cur.levelId))
                ...
                data.notStartedPercentage = 1 -
                    ((data.inProgressIds.length + data.doneIds.length) / levels.length);
                return data;
            });

            const progressionByLevels = levels.map(level => ({
                complete: progressionByStudents.filter(pbs =>
                    pbs.doneIds.includes(level.id)).length / studentList.length * 100,
                inProgress: progressionByStudents.filter(pbs =>
                    pbs.inProgressIds.includes(level.id)).length / studentList.length * 100,
            })).map((level) => {
                const newObj = level;
                newObj.notStarted = 100 - (newObj.inProgress + newObj.complete);
                return newObj;
            }).reduce((acc, cur) => {
                const newObj = acc;
    ...
    static async statsByLevel(req, res) {
        try {
            ...
            levels = await Promise.all(levels.map(async (entry) => {
                const students = await profService.students(req.user).map(user => user.id);
