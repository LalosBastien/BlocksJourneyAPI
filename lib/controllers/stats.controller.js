...
export default class StatsController {
    ...
    static async statsGlobal(req, res, next) {
        try {
            ...
            const students = await profService.ladder(req.user).map(x => x.dataValues).map(x => ({ id: x.id, nom: x.nom, prenom: x.prenom }));
            const myStudentsStats = globalHistory.filter(x => students.map(x => x.id).includes(x.userId));
            const levels = await levelService.getAll().map(x => x.dataValues);
            const studentsWithHistories = await Promise.all(Object.values(students).map(async (s) => {
                const currH = await statService.historyByUser(s.id).map(x => x.dataValues);
                if (currH && currH.length) {
                    const doneH = currH.filter(h => h.status == 'success');
                    const inProgress = currH.filter(h => h.status == 'in progress' && h.status == 'failed');
                    const doneReduced = [];
            ...
            const globalCompleteStudents = studentsWithHistories.filter(x => x.done == true);
            const globalComplete = globalCompleteStudents.length / students.length;
            const globalInProgressStudents = studentsWithHistories.filter(x => x.inProgress == true && !globalCompleteStudents.includes(x));
            ...
            Object.values(levels).map(async (l) => {
                const inProgress = globalInProgressStudents.filter(x => x.inProgressTab.includes(l.id) || x.completeTab.includes(l.id));
                const complete = globalCompleteStudents.filter(x => x.completeTab.includes(l.id));
        ...
        try {
            ...
            levels = await Promise.all(levels.map(async (entry) => {
                const history = await entry
                    .getUtilisateurs({ attributes: ['id', 'nom', 'prenom'] })
