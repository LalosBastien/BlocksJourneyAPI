...
export default class UserService extends Service {
    ...
    getUserFromProfId(profId, fields) {
        ...
        where.firstname = sequelize.where(sequelize.fn('LOWER', sequelize.col('firstname')), 'LIKE', profId.substring(0, 2) + '%');
        where.name = sequelize.where(sequelize.fn('LOWER', sequelize.col('name')), 'LIKE', profId.substring(2, 4) + '%');
        where.role = roleService.findById()
        if (fields) {
            return this.model.findOne({
                where,
                attributes: fields,
